...
namespace EmberKernel.Services.UI.Mvvm.ViewComponent.Window
{
    ...
    {
        void Register<TWindow>(ILifetimeScope scope) where TWindow : T, IHostedWindow, new();
        ValueTask RegisterAsync<TWindow>(ILifetimeScope scope, CancellationToken token = default) where TWindow : T, IHostedWindow, new();
